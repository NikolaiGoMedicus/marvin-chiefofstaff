...
- **23.04. Mittag — Tomedo DB Discovery für Arveds Auslastungs-Reporting** ✅. `besuch`-Tabelle = Tomedo-Tagesliste (nicht `termin`). Arveds 14-Tage-Zahlen matchen DB ±1 pro Zeile. Entity-Doc + 2 Reusable-Queries (`auslastung-taegliche-besuche.sql`, `auslastung-stunden-heatmap.sql`) committed + pushed (commit `16a7ff2`). [Diskussions-Sheet für Arved + Sebastian Lüttle](https://docs.google.com/spreadsheets/d/1fDMAiVdROwkzW4vzXQXLboFDpp8aAwM_FCVVo3WcE-U/edit) mit 7 Tabs erstellt. Details in [tomedo-exploration.md](projects/tomedo-exploration.md). Offen: Scope-Call mit Arved+Sebastian (DB-only Phase 1 vs. Personio parallel).
- **23.04. Nachmittag — Follow-ups zu Auslastungs-Reporting** ✅. Arved-Frage (Pat/Arzt+MFA/h aus DB) + Antonia-Frage (Post-Termin-ToDos) beide implementiert. Arzt-Kürzel aller 10 Ärzte via `kalender`-Tabelle aufgelöst (kein Personio nötig). **70–85 % aller Besuche erzeugen ≥1 Post-Termin-ToDo** (Rezept/Überweisung/AU/Folgetermin). Überweisung+AU liegen als `karteieintrag` dtype='Formular'. 3 neue Reusable-Queries im Repo (`auslastung-arztstunden-proxy`, `auslastung-mfastunden-proxy`, `post-termin-todos`), commit `0f34080` + Sebastian-Lüttles Telemed-Arbeit separat commit `a64c267`. Sheet um 2 Tabs erweitert. **Antonia-DM 15:01 gesendet** (Draft `Dr0AUXNHFVC4`).
...
- **23.04. Nachmittag — Telemed-Reporting Live-Demo mit Sebastian Lüttle** ✅ (GoMedicus Services). Korrigierte Telemed-Definition: 27 Terminarten (13 Video, 11 Sonder-GKV, 2 Telefon, 1 Privat Novo). KW13: 234 Pat gesamt, 224 GKV inkl. HZV, 102 HZV, 10 Privat. `leistung.patient_ident` bei EBM-Ziffern NULL — Fix via kvschein (Pattern-Doc angelegt). Neue Query `telemed-kontakte.sql`. Commit `30bad10`. Details in [tomedo-exploration.md](projects/tomedo-exploration.md). Offen: Arzt-Split Bisingen vollständig via Terminart-Query, Privat/GOÄ-Pfad verifizieren, Lambda-Ticket für Sebastian-Zugriff.
